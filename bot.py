...
async def cmd_genpass(message: types.Message, state: FSMContext):
    ...
    if len(args) > 1 and args[1].isdigit():
        ...
        await message.answer("❌ Ошибка, укажите длину пароля (например, /genpass 12).")
...
async def process_language(message: types.Message, state: FSMContext):
    ...
    if message.text == '1':
        ...
        await message.answer("❌ Ошибка. Пожалуйста, отправьте 1 для английского или 2 для русского")
        return
    ...
    if len(options) == 1:
        try:
            ...
            if option == 1:
                ...
                await message.answer("❌ Ошибка. Пожалуйста, отправьте 1 или 2.")
                return
        except ValueError:
            await message.answer("❌ Ошибка. Используйте цифры через пробел, например: 1 2")
            return
    else:
        try:
